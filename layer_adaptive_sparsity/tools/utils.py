...
def get_weights(model, exclude_layers):
    weights = []
    for name, m in model.named_modules():
        if _is_prunable_module(m) and name not in exclude_layers:
            weights.append((name, m.weight))
    return dict(weights)
...
def get_modules(model, exclude_layers=[]) -> dict:
    modules = []
    for name, m in model.named_modules():
        if _is_prunable_module(m) and name not in exclude_layers:
