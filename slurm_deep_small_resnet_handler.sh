...
save_folder="/jmain02/home/J2AD014/mtc03/lla98-mtc03/new_training_small_imagenet"
batch_size=128
...
sbatch --ntasks-per-node=1 --cpus-per-task=16 --nodes=1 --array=1-3 --time=144:00:00 --partition=small  --mail-type=all --mail-user=sclaam@leeds.ac.uk --error="${jname}.err" --gres=gpu:1 --output="${jname}.out"  --job-name="${jname}" slurm_deep_small_resnet_run.sh "${model}" "${dataset}" 16 ${lvl}  "normal" "${epochs}" "sgd_${epochs}_res_${res}_${ffcv_string}" 1 0 "${res}" "${ffcv}" "${ffcv_train}" "${ffcv_test}" "${save_folder}" "${batch_size}"
