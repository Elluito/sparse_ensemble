...
sbatch --nodes=1 --array=1-4 --time=140:00:00 --partition=small  --mail-type=all --mail-user=sclaam@leeds.ac.uk --error="deep_${model}_lvl_${lvl}_small_imagenet_res_${res}.err" --gres=gpu:1 --output="deep_${model}_lvl_${lvl}_small_imagenet_res_${res}.output"  --job-name="deep_${model}_lvl_${lvl}_small_imagenet_res_${res}" slurm_deep_small_resnet_run.sh "${model}" "small_imagenet" 8 ${lvl}  "normal" "${epochs}" "recording_${epochs}" 1 1 "${res}"
