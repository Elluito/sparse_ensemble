...
qsub -l h_rt=48:00:00 -t 1-10 -l coproc_v100=1 -N "SP_FT_${model}_${dataset}_sig_${sigma}_pr_${pr}_sto" arc4_SP_fine_tuning_run.sh 11 "${sigma}" "${pruner}" "${model}" "${dataset}" "${pr}" "${resolution}" "alternative" 100 "${name}" #"${save_folder}" "${name}"

qsub -l h_rt=48:00:00 -l coproc_v100=1  -N "SP_FT_${model}_${dataset}_sig_${sigma}_pr_${pr}_det" arc4_SP_fine_tuning_run.sh 6 "${sigma}" "${pruner}" "${model}" "${dataset}" "${pr}" "${resolution}" "alternative" 100 "${name}" #"${save_folder}" "${name}"
...
#for pruning_rate in pruning_rate_list; do
