...
if [ "${ffcv}" -gt 0 ]
  then
  echo "Use FFCV"
qsub -l h_rt=48:00:00 -l coproc_v100=1  -N  "${model}_${rf_level}_${dataset}_${pruning_rate}_pruning_ffcv" arc4_pruning_run.sh FFCV="${ffcv}" NAME="${name}" MODEL="${model}" DATASET="${dataset}"  NUMW=16  RFL="${rf_level}" TYPE="normal" FOLDER="${directory}" PR="${pruning_rate}" EXPERIMENT=1 FFCV_TRAIN="${ffcv_train}" FFCV_VAL="${ffcv_val}" DATA_FOLDER="${data_folder}" SAVE_FOLDER="${save_folder}" INPUT_RES="${resolution}" RESIZE="${resize}"

else
 echo "Don't use FFCV"
 qsub -l h_rt=48:00:00 -l coproc_v100=1  -N "${model}_${rf_level}_${dataset}_${pruning_rate}_pruning_no_ffcv" arc4_pruning_run.sh FFCV="${ffcv}" NAME="${name}" MODEL="${model}" DATASET="${dataset}"  NUMW=16  RFL="${rf_level}" TYPE="normal" FOLDER="${directory}" PR="${pruning_rate}" EXPERIMENT=1 DATA_FOLDER="${data_folder}" SAVE_FOLDER="${save_folder}" INPUT_RES="${resolution}" RESIZE="${resize}"
  fi
#done
#done
...
checkpoints_folder="/nobackup/sclaam/deep_small_modelsresized"
...
run_pruning "${model}" "${dataset}" "${checkpoints_folder}" "/nobackup/sclaam/data" "${save_folder}" "sgd_100_res_224_no_ffcv" 0 "/jmain02/home/J2AD014/mtc03/lla98-mtc03/small_imagenet_ffcv/train_360_0.5_90.ffcv" "/jmain02/home/J2AD014/mtc03/lla98-mtc03/small_imagenet_ffcv/val_360_0.5_90.ffcv" "${pruning_rate}" "${rf_level}" "${resolution}" "${resize}"
