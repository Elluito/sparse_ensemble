...
python3.9 prune_models.py --ffcv --name "${name}" --model "${model}" --dataset "${dataset}" --num_workers 0 --RF_level "${rf_level}" --type "normal" --folder "${directory}" --pruning_rate "${pruning_rate}"  --experiment 8 --data_folder "${data_folder}" --save_folder "${save_folder}" --input_resolution  "${resolution}" --resize  "${resize}"
else

python3.9 prune_models.py --name "${name}" --model "${model}" --dataset "${dataset}" --num_workers 0 --RF_level "${rf_level}" --type "normal" --folder "${directory}" --pruning_rate "${pruning_rate}"  --experiment 8 --data_folder "${data_folder}" --save_folder "${save_folder}" --input_resolution  "${resolution}" --resize  "${resize}"
...
###############################
#    resnet50 ekfac level 1 lr 0.1 momentum 0.9
###############################
...
#save_folder="${HOME}/sparse_ensemble/second_order_pruning_lr_0.1_momentum_0.9"
...
#for pruning_rate in "0.6" "0.8" "0.9" ; do
for pruning_rate in "0.9"; do
for rf_level in "1"; do

run_pruning "${model}" "$dataset" "${solutions_folder}" "${data_folder}" "${save_folder}" "${name}" 0 "/jmain02/home/J2AD014/mtc03/lla98-mtc03/small_imagenet_ffcv/train_360_0.5_90.ffcv" "/jmain02/home/J2AD014/mtc03/lla98-mtc03/small_imagenet_ffcv/val_360_0.5_90.ffcv" "${pruning_rate}" "${rf_level}" "${resolution}" "${resize}"
done
done
done
done
