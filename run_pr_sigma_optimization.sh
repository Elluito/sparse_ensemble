...
#qsub -N "search_sigma_pr_MOO_${model}_${dataset}_${sampler}" run.sh  19 "0.0" "global" "${model}" "${dataset}" "0.9" "alternative" "1" "${sampler}" "2"
./run.sh  19 "0.0" "global" "${model}" "${dataset}" "0.9" "alternative" "1" "${sampler}" "2"
./run.sh  19 "0.0" "global" "resnet18" "cifar10" "0.9" "alternative" "1" "nsga" "2"
#done
done
done
