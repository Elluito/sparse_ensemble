...
from HRankPlus.models.cifar10.googlenet import Inception
from HRankPlus.models.cifar10.vgg import vgg_16_bn
from HRankPlus.models.cifar10.resnet import resnet_56, resnet_110

from HRankPlus.data import cifar10
import HRankPlus.utils.common as utils
        ...
        if isinstance(module, Inception):
            ...
            for weight_index in honey_channel_index:
                ...
                if branch_name == '_pool_planes':
                    tmp_select_index = list(range(state_dict[name_base + conv_name].size(0)))
                    tmp_select_index = [x + filters[cov_id - 2][0] + filters[cov_id - 2][1] + filters[cov_id - 2][2] for
                                        x in tmp_select_index]
                    cur_last_select_index += tmp_select_index
...
def load_pretrained_model_from_config(args):
    ...
    # return model
    print("About to create {} model".format(args.architecture))
    origin_model = eval(args.architecture)(compress_rate=[0.] * 100)
    print("DONE")
    print("About to load pre trained solution {} model".format(args.architecture))
    state_dict = torch.load(args.pre_trained_solution, map_location="cpu")["state_dict"]
    print("DONE")
    origin_model.load_state_dict(state_dict)

    return origin_model.cuda()
