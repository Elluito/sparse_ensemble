...
parser.add_argument(
    ...
    default='vgg_16_bn',
    choices=(
    'resnet_50', 'vgg_16_bn', 'resnet_56', 'resnet_110', 'densenet_40', 'googlenet', 'mobilenet_v2', 'mobilenet_v1'),
...
if args.arch == 'vgg_16_bn':
    ...
    for i in range(3):
        ...
        for j in range(12):
            ...
            np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv%d' % (13 * i + j + 1) + '.npy',
                    feature_result.numpy())
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)
        ...
        if i < 2:
            ...
            np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv%d' % (13 * (i + 1)) + '.npy',
                    feature_result.numpy())
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)  #
    ...
    inference()
    handler.remove()
    np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv%d' % (39) + '.npy',
            feature_result.numpy())
    feature_result = torch.tensor(0.)
    total = torch.tensor(0.)
    ...
    for idx, cov in enumerate(cov_list):
        ...
        if idx > 0:
            for idx1, tp in enumerate(tp_list):
                if idx1 == 3:
                    np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv%d_' % (
                                idx + 1) + tp + '.npy',
            ...
            np.save('rank_conv/' + args.arch + '_limit%d' % (args.limit) + '/rank_conv%d' % (idx + 1) + '.npy',
                    feature_result.numpy())
        feature_result = torch.tensor(0.)
        total = torch.tensor(0.)
