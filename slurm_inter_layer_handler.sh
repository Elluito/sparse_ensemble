...
if [ "${ffcv}" -gt 0 ]
  ...
  sbatch --nodes=1 --time=03:00:00 --partition=small --gres=gpu:1 --mail-type=ALL --mail-user=sclaam@leeds.ac.uk  --error="${model}_${rf_level}_${dataset}_${pruning_rate}_intermediate_layer_pruning_ffcv.err" --output="${model}_${rf_level}_${dataset}_${pruning_rate}_intermediate_layer_pruning_ffcv.out" --job-name="${model}_${rf_level}_${dataset}_${pruning_rate}_intermediate_layer_pruning_ffcv" slurm_inter_layer_run.sh FFCV="${ffcv}" NAME="${name}" MODEL="${model}" DATASET="${dataset}"  NUMW=4  RFL="${rf_level}" TYPE="normal" FOLDER="${directory}" PR="${pruning_rate}" EXPERIMENT=5 FFCV_TRAIN="${ffcv_train}" FFCV_VAL="${ffcv_val}" DATA_FOLDER="${data_folder}" SAVE_FOLDER="${save_folder}"

else

 echo "Don't use FFCV"

 sbatch --nodes=1 --time=03:00:00 --partition=small --gres=gpu:1 --mail-type=ALL --mail-user=sclaam@leeds.ac.uk  --error="${model}_${rf_level}_${dataset}_${pruning_rate}_intermediate_layer_pruning_no_ffcv.err" --output="${model}_${rf_level}_${dataset}_${pruning_rate}_intermediate_layer_pruning_no_ffcv.out" --job-name="${model}_${rf_level}_${dataset}_${pruning_rate}_intermediate_layer_pruning_no_ffcv" slurm_inter_layer_run.sh FFCV="${ffcv}" NAME="${name}" MODEL="${model}" DATASET="${dataset}"  NUMW=4  RFL="${rf_level}" TYPE="normal" FOLDER="${directory}" PR="${pruning_rate}" EXPERIMENT=5 DATA_FOLDER="${data_folder}" SAVE_FOLDER="${save_folder}"
  fi
