...
sbatch --nodes=1 --time=47:00:00 --array=1-4 --partition=gpu  --mail-type=all --mail-user=sclaam@leeds.ac.uk --error="dilation_${model}_lvl_${lvl}_${dataset}.err" --gres=gpu:1 --output="dilation_${model}_lvl_${lvl}_${dataset}.out"  --job-name="dilation_${model}}_lvl_${lvl}_${dataset}" slurm_dilation_experiments_run.sh "${model}" "${dataset}" 8 ${lvl}  "normal" 200 "recording_dilation_200_no_ffcv" 1 1 ${resolution}
done
done
done
