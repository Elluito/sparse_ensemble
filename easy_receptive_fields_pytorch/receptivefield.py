...
def give_effective_receptive_field(net, input_shape, device='cpu'):
    ...
    try:
        ...
        if device.type != param_device.type or (device.index is not None and device.index != param_device.index):
            raise ValueError(
                "The input network's device and the receptive field function's device argument must be the same.")
    ...
    def insert_hook(module):
        ...
        #   if len(module)==0:
        #     hook =_passthrough_grad # _sequential_passthrough_grad
    ...
    input = t.ones(input_shape, requires_grad=True, device=device) * 3
    output = net(input)
...
def receptivefield(net, input_shape, device='cpu'):
    ...
    # get receptive field bounding box, to compute its size.
    # the position of the one-hot output gradient (pos) is stored for later.
    (x1, x2, y1, y2, pos) = _project_rf(input, output, return_pos=True, net=net)
    rfsize = Size(x2 - x1 + 1, y2 - y1 + 1)
...
def _project_rf(input, output, offset_x=0, offset_y=0, return_pos=False, net=None, return_grad=False):
    ...
    pos2[x_dim] = math.ceil(output.shape[x_dim] / 2) - 1 + offset_x
    pos2[y_dim] = math.ceil(output.shape[y_dim] / 2) - 1 + offset_y
    input_pos = [math.ceil(input.shape[x_dim] / 2) - 1 + offset_x, math.ceil(input.shape[y_dim] / 2) - 1 + offset_y]
...
def _sequential_passthrough_grad(self, grad_input, grad_output):
    return (t.ones_like(grad_input[0], requires_grad=True),)
